...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps {
  ...
  NSString *identifier =
      RCTNSStringFromStringNilIfEmpty(newProps.casId) ?: CASMobileAds.casIdendifier;

  NSString *placement = RCTNSStringFromStringNilIfEmpty(newProps.placement);

  CASSize *adSize = RNCASSizeWithType(newProps.sizeConfig.sizeType[0], newProps.sizeConfig.maxWidth,
  ...
  if (self.bannerView) {
    ...
    firstLoad = YES;
    self.bannerView = [[CASBannerView alloc] initWithCasID:identifier
                                                      size:adSize
                                                    origin:CGPointZero];
...
- (void)bannerAdViewDidLoad:(CASBannerView *)view {
  if (_eventEmitter != nullptr) {
    ...
    std::dynamic_pointer_cast<const CASAdViewEventEmitter>(_eventEmitter)
        ->onAdViewLoaded(
            CASAdViewEventEmitter::OnAdViewLoaded{.width = size.width, .height = size.height});
  }
}
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const CASAdViewEventEmitter>(_eventEmitter)
        ->onAdViewImpression(CASAdViewEventEmitter::OnAdViewImpression{
            ...
            .revenuePrecision =
                RCTStringFromNSString(RNCASNSStringFromRevenuePresision(info.revenuePrecision)),
...
Class<RCTComponentViewProtocol> RNCASBannerAdViewCls(void) { return RNCASBannerAdView.class; }
